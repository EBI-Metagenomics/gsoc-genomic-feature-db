...
class TestDataIntegrity:
    ...
    def test_multiple_chromosomes(self, conn):
        seqids = {
            r[0] for r in conn.execute("SELECT DISTINCT seqid FROM features").fetchall()
        }
    ...
    def test_strand_values_valid(self, conn):
        strands = {
            r[0]
            for r in conn.execute("SELECT DISTINCT strand FROM features").fetchall()
        }
        assert strands <= {"+", "-", "."}
...
class TestFTS:
    ...
    def test_fts_row_count_matches(self, conn):
        ...
        fts_count = conn.execute("SELECT count(*) FROM features_fts").fetchone()[0]
    ...
    def test_search_by_feature_type(self, conn):
        results = conn.execute(
            "SELECT count(*) FROM features_fts " "WHERE features_fts MATCH '\"gene\"*'"
        ).fetchone()[0]
        assert results >= 1
    ...
    def test_prefix_search(self, conn):
        ...
        results = conn.execute(
            "SELECT count(*) FROM features_fts " "WHERE features_fts MATCH '\"OR4F\"*'"
        ).fetchone()[0]
        assert results >= 2
