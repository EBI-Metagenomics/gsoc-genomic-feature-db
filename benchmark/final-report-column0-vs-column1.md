...
|---|---|---:|---:|---:|---|
| small | Indexing time | 2.01 s | 1.93 s | -4.0% | within threshold |
| small | Peak indexing memory | 131.52 MiB | 148.31 MiB | +12.8% | within threshold |
| small | Database size | 17.70 MiB | 15.91 MiB | -10.1% | within threshold |
| small | Initial browser load p95 | 2539.9 ms | 7075.2 ms | +178.6% | explanation required |
| small | Initial bytes transferred p95 | 0.03 MiB | 0.03 MiB | +0.0% | within threshold |
| small | Visible search latency p95 | 127.7 ms | 122.2 ms | -4.3% | within threshold |
| small | Search bytes transferred p95 | 0.14 MiB | 0.14 MiB | +0.0% | within threshold |
| small | Browser JS heap p95 | 36.63 MiB | 42.09 MiB | +14.9% | within threshold |
| small | Main-thread long-task duration p95 | 0.0 ms | 0.0 ms | +0.0% | within threshold |
| medium | Indexing time | 10.28 s | 10.08 s | -1.9% | within threshold |
| medium | Peak indexing memory | 451.00 MiB | 439.13 MiB | -2.6% | within threshold |
| medium | Database size | 130.13 MiB | 124.58 MiB | -4.3% | within threshold |
| medium | Initial browser load p95 | 2425.6 ms | 7001.5 ms | +188.7% | explanation required |
| medium | Initial bytes transferred p95 | 0.03 MiB | 0.03 MiB | +0.0% | within threshold |
| medium | Visible search latency p95 | 229.5 ms | 227.1 ms | -1.0% | within threshold |
| medium | Search bytes transferred p95 | 2.02 MiB | 2.03 MiB | +0.4% | within threshold |
| medium | Browser JS heap p95 | 44.71 MiB | 46.55 MiB | +4.1% | within threshold |
| medium | Main-thread long-task duration p95 | 0.0 ms | 0.0 ms | +0.0% | within threshold |
| large | Indexing time | 152.69 s | 149.28 s | -2.2% | within threshold |
| large | Peak indexing memory | 1891.86 MiB | 1765.18 MiB | -6.7% | within threshold |
| large | Database size | 1253.91 MiB | 1139.92 MiB | -9.1% | within threshold |
| large | Initial browser load p95 | 2142.4 ms | 7322.5 ms | +241.8% | explanation required |
| large | Initial bytes transferred p95 | 0.03 MiB | 0.03 MiB | +0.0% | within threshold |
| large | Visible search latency p95 | 860.6 ms | 866.5 ms | +0.7% | within threshold |
| large | Search bytes transferred p95 | 12.04 MiB | 12.04 MiB | +0.0% | within threshold |
| large | Browser JS heap p95 | 36.32 MiB | 36.89 MiB | +1.6% | within threshold |
| large | Main-thread long-task duration p95 | 0.0 ms | 0.0 ms | +0.0% | within threshold |

## Unexpected or negative results

- `small.initial_load_p95` (+178.6%): **TODO: add an evidence-based explanation.**
- `medium.initial_load_p95` (+188.7%): **TODO: add an evidence-based explanation.**
- `large.initial_load_p95` (+241.8%): **TODO: add an evidence-based explanation.**
...
- [ ] Regressions above 20% have evidence-based explanations.
